...
-- this subquery find the most spending and least spending customers

-- CTE to get all columns from merchants
with most_spent as (select * from merchants)

-- getting merchant name, customer name, and the spending they had
select most_spent.name, customers.fullname, format(sum(price * quantity_available), 2) as max_total_spent
from most_spent
join sell on most_spent.mid = sell.mid
join products on sell.pid = products.pid
join contain on products.pid = contain.pid
join orders on contain.oid = orders.oid
join place on orders.oid = place.oid
join customers on place.cid = customers.cid
group by most_spent.name, customers.fullname
having max_total_spent >= all ( -- max total spent is greater than all others per merchant
	select format(sum(price * quantity_available), 2)
    from merchants
    join sell on most_spent.mid = sell.mid
	join products on sell.pid = products.pid
	join contain on products.pid = contain.pid
	...
	join place on orders.oid = place.oid
	join customers on place.cid = customers.cid
    where most_spent.name = merchants.name
    group by merchants.name, customers.fullname
) or max_total_spent <= all ( -- max total spent is less than all others per merchant
	select format(sum(price * quantity_available), 2)
    from merchants
    join sell on most_spent.mid = sell.mid
	join products on sell.pid = products.pid
...
order by most_spent.name, customers.fullname;
